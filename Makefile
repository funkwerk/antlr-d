...
.PHONY: build_library
build_library: $(BUILD_DIR)/libantlr-d.so.4.9.2

$(BUILD_DIR)/libantlr-d.so.4.9.2: $(SOURCE_FILES)
	$(DMD) -shared -relocation-model=pic -op -Hd=$(BUILD_DIR)/di $(SOURCE_FILES) \
		-od=$(BUILD_DIR) -of=$(BUILD_DIR)/libantlr-d.so.4.9.2
...
.PHONY: install
install: $(BUILD_DIR)/libantlr-d.so.4.9.2
	cp $(BUILD_DIR)/libantlr-d.so.4.9.2 $(EXPORT_LIB)
	rm -rf $(EXPORT_LIB)/libantlr-d.so.4 $(EXPORT_LIB)/libantlr-d.so
	ln -s $(EXPORT_LIB)/libantlr-d.so.4.9.2 $(EXPORT_LIB)/libantlr-d.so.4
