...
tfpdef:
      ...
      | {isTTS}? NUMBER      # tfpdef_number
      | {isTTS}? RULE_STRING # tfpdef_string
      | {isTTS}? funct_stmt  # tfpdef_funct_stm
      ;
...
var_stmt: dotted_name (dot_e funct_stmt)?;
...
compound_stmt: if_stmt | for_stmt | block_stmt;
...
block_suite: NEWLINE INDENT (string_stmt+ NEWLINE)+ DEDENT;

test: and_test (NEWLINE? or_e and_test)*;
or_e: OR;
...
expr: xor_expr (NEWLINE? '|' xor_expr)* | dotted_name;
xor_expr: and_expr (NEWLINE? '^' and_expr)*;
and_expr: arith_expr (NEWLINE? '&' arith_expr)*;
arith_expr: factor (NEWLINE? (add|minus) factor)*;
factor: (NEWLINE? ('+'|'-')) factor | atom;

atom:
      dotted_name   # atom_dotted_name
    | funct_stmt    # atom_funct_stmt
    | NUMBER        # number_e
    | RULE_STRING   # string_e
    | TRUE          # true_e
    | FALSE         # false_e
    | LAST          # last_e
    | FIRST         # first_e
...
templatesBegin: (NEWLINE | funcdef)*
          EOF;
